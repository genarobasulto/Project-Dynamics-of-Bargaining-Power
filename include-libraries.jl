using JuMP, Ipopt              # Loading optimization libraries.
                               # Ipopt solve a nonlinear optimization problem.
using DataFrames               # Loading frame libraries.
using ExcelFiles               # Loading excel files libraries.
using XLSX
using Random                   #Loading random library for the simulations. 
using Plots,Plots.PlotMeasures #Loading plotting libraries.  
using Colors, ColorSchemes     #Loading color libraries for plotting.